...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor }) {
  ...
  if ((printInfo || []).length > 0) {
    ...
    doc.text('PRINT DETAILS', margin + 14, y + 17);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(138, 146, 176);
    doc.text(`${totalLocations} location${totalLocations !== 1 ? 's' : ''}  \u00B7  ${totalColors} color${totalColors !== 1 ? 's' : ''}`, pageW - margin - 14, y + 17, { align: 'right' });

    y += 40;

    // ── Print placements ──
    for (const p of (printInfo || [])) {
      if (needsNewPage(72)) { drawFooter(); doc.addPage(); y = margin; }
      ...
      doc.roundedRect(margin, y, 4, 58, 2, 2, 'F');

      // Placement name
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.setTextColor(30, 34, 50);
      doc.text(p.placement, margin + 18, y + 20);

      // Print size
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.setTextColor(30, 34, 50);
      doc.text(`${p.widthInches}" \u00D7 ${p.heightInches}"`, pageW - margin - 14, y + 20, { align: 'right' });

      // Color swatches
      let swatchX = margin + 18;
      const swatchY = y + 32;
      for (const c of (p.colors || [])) {
        ...
        doc.roundedRect(swatchX, swatchY, pillW, pillH, 9, 9, 'F');

        if (light) {
          doc.setDrawColor(190, 190, 200);
          doc.setLineWidth(0.5);
          doc.roundedRect(swatchX, swatchY, pillW, pillH, 9, 9, 'S');
        }

        doc.setTextColor(light ? 40 : 255, light ? 40 : 255, light ? 40 : 255);
        doc.text(c.name, swatchX + pillW / 2, swatchY + pillH / 2 + 3, { align: 'center' });
