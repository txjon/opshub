...
// opts.keepHiddenLocations (proof-spec path only): if a group's VISIBLE layers
// yield no bounds — e.g. the whole print is toggled off in the PSD — fall back
// to the hidden layers' geometry so the print LOCATION still lands on the proof
// instead of silently vanishing. The mockup path leaves this off, so a hidden
// print never renders into a mockup.
export function extractPrintInfoFromPsd(psd, opts = {}) {
  const keepHiddenLocations = opts.keepHiddenLocations === true;
  const info = [];
  ...
  for (const group of groups) {
    ...
    // Fallback bounds/colors that INCLUDE hidden layers (still excluding non-ink
    // and truly empty layers). Only consulted when keepHiddenLocations is set
    // and the visible pass found nothing.
    let fL = Infinity, fT = Infinity, fR = -Infinity, fB = -Infinity;
    const colors = [];
    const colorsInclHidden = [];
    ...
    for (const layer of (group.children || [])) {
      ...
      // Fallback pass: an inked layer counts even if hidden.
      if (keepHiddenLocations && !isNonInk && !isBlank) {
        fL = Math.min(fL, layer.left || 0);
        fT = Math.min(fT, layer.top || 0);
        fR = Math.max(fR, layer.right || 0);
        fB = Math.max(fB, layer.bottom || 0);
        colorsInclHidden.push({ name: layer.name, hex: isBase ? '#ffffff' : sampleLayerColor(layer.canvas) });
      }
    ...
    // Whole location toggled off? Recover it from the hidden-layer geometry so
    // the proof still lists the print (mockup path never sets this flag).
    if ((artW <= 0 || artH <= 0 || !isFinite(artW) || !isFinite(artH)) && keepHiddenLocations) {
      const fW = fR - fL, fH = fB - fT;
      if (isFinite(fW) && isFinite(fH) && fW > 0 && fH > 0) {
        minL = fL; minT = fT; maxR = fR; maxB = fB;
        artW = fW; artH = fH;
        if (colors.length === 0) outColors = colorsInclHidden;
      }
    }
