 ...
 */
export async function registerFileInDb({ fileId, webViewLink, fileName, mimeType, fileSize, itemId, stage }) {
  const res = await fetch("/api/drive/register", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ fileId, webViewLink, fileName, mimeType, fileSize, itemId, stage }),
  });
  if (!res.ok) {
