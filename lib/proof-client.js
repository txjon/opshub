...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes, tenantSlug, tenantName }) {
  ...
  if ((printInfo || []).length > 0) {
    ...
    for (const p of (printInfo || [])) {
      ...
      // Calculate row height \u2014 swatches now render as a color chip
      // with the PMS name beneath (Pantone-card style, no pill), so
      // each color cell needs chip + gap + label-line of vertical
      // space rather than the old 14pt pill.
      let rowH = 20;
      if (sizeStr !== '\u2014') rowH += 10;
      if (p.callout) rowH += 10;
      if (colorCount > 0) rowH += 24;
      ...
      // Color swatches — Pantone-card style: small filled square
      // chip with the PMS name in mono-ish text below it. No pill
      // around the name; the chip carries the color, the text is
      // just data. Each cell wraps to a new row when the line is
      // full, and the parent rowH grows accordingly so the next
      // print location pushes down cleanly.
      if (colorCount > 0) {
        const chipSize = 12;
        const labelGap = 3;
        const labelFontSize = 6.5;
        const labelLineH = 8;
        const cellVStride = chipSize + labelGap + labelLineH; // ~23
        const cellHGap = 6;
        ...
        for (const c of sortedColors) {
          ...
          // fall back to a neutral gray chip rather than crashing.
          ...
          const cellW = Math.max(chipSize, nameW);

          // Wrap to next row if this cell would overflow the column.
          if (chipX + cellW > colEndX && chipX > col2X) {
            chipX = col2X;
            chipY += cellVStride + 4;
            extraRows++;
          }
          ...
          const g = parseInt(hex.slice(3, 5), 16);
          const b = parseInt(hex.slice(5, 7), 16);
          const chipDrawX = chipX + (cellW - chipSize) / 2;

          doc.setFillColor(r, g, b);
          doc.roundedRect(chipDrawX, chipY, chipSize, chipSize, 2, 2, 'F');
          if (light) {
            doc.setDrawColor(200, 200, 208);
            doc.setLineWidth(0.5);
            doc.roundedRect(chipDrawX, chipY, chipSize, chipSize, 2, 2, 'S');
          }

          // Label centered under the chip — plain text, no background.
          doc.setTextColor(80, 80, 90);
          doc.text(c.name, chipX + cellW / 2, chipY + chipSize + labelGap + labelFontSize - 0.5, { align: 'center' });

          chipX += cellW + cellHGap;
        }

        if (extraRows > 0) rowH += extraRows * (cellVStride + 4);
      } else {
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.setTextColor(102, 102, 102);
