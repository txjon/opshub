...
// ── Light theme ──
const C = {
  bg: "#f5f6fa", card: "#ffffff", border: "#e2e5ee",
  text: "#1a1d2b", muted: "#6b7089", faint: "#9ca3b8",
  accent: "#4361ee", accentBg: "#eef1ff",
...
const STAGE_LABELS: Record<string, { label: string; bg: string; color: string }> = {
  pending: { label: "PO Sent", bg: C.accentBg, color: C.accent },
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: C.font, color: C.text }}>
      ...
      <div style={{
        ...
        <div style={{ color: "#8a92b0", fontSize: 11 }}>Vendor Portal</div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: isMobile ? "20px 16px 60px" : "32px 24px 60px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: isMobile ? 22 : 28, fontWeight: 700, margin: "0 0 4px" }}>{decorator.name}</h1>
        ...
        <div style={{
          ...
          {needsAction > 0 && (
            <div style={{ padding: "8px 16px", borderRadius: 8, background: C.accentBg, border: `1px solid ${C.accent}33` }}>
              <span style={{ fontSize: 20, fontWeight: 700, color: C.accent }}>{needsAction}</span>
              <span style={{ fontSize: 12, color: C.accent, marginLeft: 6 }}>needs confirmation</span>
            </div>
          )}
          {inProd > 0 && (
            <div style={{ padding: "8px 16px", borderRadius: 8, background: C.amberBg, border: `1px solid ${C.amber}33` }}>
              <span style={{ fontSize: 20, fontWeight: 700, color: C.amber }}>{inProd}</span>
              <span style={{ fontSize: 12, color: C.amber, marginLeft: 6 }}>in production</span>
            </div>
          )}
        </div>
      ...
      {confirmAction && (
        <div style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            background: C.card, borderRadius: 12, padding: 24, maxWidth: 380, width: "90%",
