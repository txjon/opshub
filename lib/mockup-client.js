...
export async function buildMockupClient(psdArrayBuffer) {
  ...
  // Layer names that are template helpers, not real separations. "Base" is
  // intentionally NOT here — it's a real white underlayer pull and belongs
  // on the proof as its own separation.
  ...
  for (const group of groups) {
    ...
    for (const layer of group.children) {
      ...
      // Bounds rule: only real inked layers (those with a rendered canvas)
      // drive the measured print size. Blank Pantone-callout layers can sit
      // outside the ink area on small prints like Left Chest — their stored
      // bounds would wrongly inflate the size. They still list as separations
      // below so the printer sees them.
