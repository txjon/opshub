...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor }) {
  ...
  // ── Info cards ──
  const infoCols = [
    { label: 'CLIENT', value: clientName || '\u2014', flex: 1.4 },
    { label: 'ITEM', value: itemName || '\u2014', flex: 1 },
    { label: 'BLANK', value: blankStr, flex: 1.6 },
  ];
  ...
  if ((printInfo || []).length > 0) {
    ...
    for (const p of (printInfo || [])) {
      if (needsNewPage(56)) { drawFooter(); doc.addPage(); y = margin; }

      // Card background
      doc.setFillColor(248, 249, 252);
      doc.roundedRect(margin, y, contentW, 49, 5, 5, 'F');

      // Left accent bar
      doc.setFillColor(30, 34, 50);
      doc.roundedRect(margin, y, 4, 49, 2, 2, 'F');
      ...
      doc.setFontSize(11);
      doc.setTextColor(30, 34, 50);
      doc.text(`${p.widthInches}" \u00D7 ${p.heightInches}"`, pageW - margin - 12, y + 16, { align: 'right' });
      ...
      for (const c of (p.colors || [])) {
        ...
        doc.roundedRect(swatchX, swatchY, pillW, pillH, 7, 7, 'F');

        if (light) {
          doc.setDrawColor(190, 190, 200);
          doc.setLineWidth(0.5);
          doc.roundedRect(swatchX, swatchY, pillW, pillH, 7, 7, 'S');
        }

        doc.setTextColor(light ? 40 : 255, light ? 40 : 255, light ? 40 : 255);
        doc.text(c.name, swatchX + pillW / 2, swatchY + pillH / 2 + 2.5, { align: 'center' });
