const { generatePDF } = require('./pdf/browser');
...
function renderProofHTML(opts) {
  ...
  const placements = (printInfo || []).map(p => {
    const swatches = (p.colors || []).map(c => {
      const r = parseInt(c.hex.slice(1, 3), 16);
      const g = parseInt(c.hex.slice(3, 5), 16);
      const b = parseInt(c.hex.slice(5, 7), 16);
      const bright = (r * 299 + g * 587 + b * 114) / 1000;
      const textColor = bright > 150 ? '#222' : '#fff';
      const border = bright > 150 ? '1px solid #ccc' : 'none';
      return `<span style="display:inline-flex;align-items:center;gap:4px;font-size:9.5px;padding:3px 10px;border-radius:20px;background:${c.hex};color:${textColor};border:${border}">
        <span style="width:10px;height:10px;border-radius:50%;background:${c.hex};border:1.5px solid ${bright > 150 ? '#aaa' : 'rgba(255,255,255,0.4)'};flex-shrink:0"></span>
        ${c.name}
      </span>`;
    }).join(' ');
    ...
    return `<div style="border-left:3px solid #1a1a1a;padding-left:16px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px">
        <div style="font-size:13px;font-weight:700">${p.placement}</div>
        <div style="font-size:10px;color:#888;font-family:${mono}">${p.widthInches}" × ${p.heightInches}"</div>
      </div>
...
<div style="background:#fff;font-family:${font};color:#111;max-width:780px;margin:0 auto">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:18px;border-bottom:2px solid #1a1a1a;margin-bottom:18px">
    ...
    <div style="text-align:right">
      <div style="font-size:20px;font-weight:800;letter-spacing:-0.5px;color:#1a1a1a">Print Proof</div>
      <div style="font-size:10px;color:#888;margin-top:4px">${today}</div>
    </div>
  </div>
  ...
  <div style="display:flex;gap:0;border:0.5px solid #ccc;margin-bottom:16px">
    ...
    ].map(([k, v], i, arr) => `<div style="flex:1;padding:5px 8px;${i < arr.length - 1 ? 'border-right:0.5px solid #ccc' : ''}">
  ...
  ${mockupBase64 ? `<div style="text-align:center;margin-bottom:20px;background:#f7f7f7;padding:20px;border-radius:4px">
    <img src="data:image/jpeg;base64,${mockupBase64}" style="max-width:100%;height:auto" />
  </div>` : ''}

  <div style="background:#222;color:#fff;padding:5px 10px;display:flex;gap:24px;font-size:9.5px;margin-bottom:20px">
    <div><span style="opacity:0.6;margin-right:4px;text-transform:uppercase;font-size:8.5px;letter-spacing:0.05em">Locations</span>${(printInfo || []).length}</div>
    <div><span style="opacity:0.6;margin-right:4px;text-transform:uppercase;font-size:8.5px;letter-spacing:0.05em">Total colors</span>${(printInfo || []).reduce((a, p) => a + (p.colors?.length || 0), 0)}</div>
  </div>

  ${placements}

  <div style="border-top:0.5px solid #ddd;padding-top:10px;font-size:7.5px;color:#aaa;line-height:1.6">
    <strong style="font-size:8px;font-weight:700;color:#888;display:block;margin-bottom:3px">Print Proof — House Party Distro</strong>
...
async function generateProofPdf(opts) {
  const html = renderProofHTML(opts);
