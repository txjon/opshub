...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor }) {
  ...
  if (mockupDataUrl) {
    const imgW = contentW;
    const imgH = contentW / 2;
    doc.addImage(mockupDataUrl, 'JPEG', margin, y, imgW, imgH);
    y += imgH + 16;
  }
  ...
  for (const p of (printInfo || [])) {
    ...
    // Print size — larger and prominent
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(13);
    doc.setTextColor(100, 90, 220);
  ...
  doc.setFontSize(7);
  doc.setTextColor(160, 165, 185);
  doc.text('House Party Distro', pageW - margin, y, { align: 'right' });
