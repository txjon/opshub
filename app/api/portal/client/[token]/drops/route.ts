...
// Client Hub — DROPS (releases, mig 134). The client-side drop builder:
// gather product lines from ACROSS their ideas into one dated release.
// GET → their drops with slots + per-slot readiness (contributing idea
// approved?). POST → start a drop. 'studio' grant required.
// (Route named /drops — /releases is taken by the legacy staging board.)
...
async function ctxOf(token: string) {
  ...
  const denied = !Array.isArray((client as any).portal_features) || !(client as any).portal_features.includes("studio");
