...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[QB Webhook1] Events:", events.map(e => `${e.name}:${e.id}@${e.realmId}`).join(", ") || "(none)");
...
async function processPayment(payment: any, supabase: any, paymentId: string) {
  ...
  console.log("[QB Webhook1] Payment total:", totalAmt, "CustomerRef:", payment.CustomerRef?.value);
  ...
  console.log("[QB Webhook1] Linked invoices:", allocations.map(a => `${a.invoiceId}=${a.amount}`).join(", ") || "NONE");
