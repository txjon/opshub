 ...
 */
export async function uploadToDrive({ blob, fileName, mimeType, itemId, clientName, projectTitle, itemName, onProgress }) {
  // Step 1: Get token + folder from server. itemId is preferred — the
  // route reads stashed Drive folder ids off the item/job/client rows,
  // so a memo rename doesn't split files into a sibling folder. Name
  // strings are kept as a fallback for the toolkit (no item row).
  const tokenRes = await fetch("/api/drive/token", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ itemId, clientName, projectTitle, itemName }),
  });
  if (!tokenRes.ok) {
  ...
  // NOT shared with anyone-with-link. OpsHub serves every file through its own
  // routes, which judge the viewer (lib/file-access). Granting public access
  // here re-opened the archive on every single upload, one file at a time.
 ...
 */
export async function registerFileInDb({ fileId, webViewLink, folderLink, fileName, mimeType, fileSize, itemId, stage, notes, preserveApproval }) {
  const res = await fetch("/api/drive/register", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ fileId, webViewLink, folderLink, fileName, mimeType, fileSize, itemId, stage, notes, preserveApproval }),
  });
  if (!res.ok) {
