...
type Pay = { contractor_id: string; hourly_rate: number; qb_vendor_name: string | null };
...
export function ContractorHoursView() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: cs }, { data: es }, { data: ps }, { data: rs }] = await Promise.all([
      ...
      supabase.from("contractor_pay").select("contractor_id, hourly_rate, qb_vendor_name"),
  ...
  const rows = useMemo(() => contractors.map(c => {
    ...
    return { c, hours, rate, amount: Math.round(hours * rate * 100) / 100, openShift, run, vendor: pay[c.id]?.qb_vendor_name };
      ...
      {loading ? <div style={{ color: T.muted, fontSize: 12, padding: 12 }}>Loading…</div> : (
        <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: "hidden" }}>
          <div style={{ display: "flex", gap: 12, padding: "8px 16px", background: T.surface, ...lbl }}>
            <span style={{ flex: 1 }}>Contractor</span><span style={{ width: 90, textAlign: "right" }}>Hours</span>
            <span style={{ width: 110, textAlign: "right" }}>Rate</span><span style={{ width: 110, textAlign: "right" }}>Amount</span>
            <span style={{ width: 200, textAlign: "right" }}>QuickBooks</span>
          </div>
          {rows.length === 0 && <div style={{ padding: 14, fontSize: 12, color: T.muted }}>No active contractors.</div>}
          {rows.map(({ c, hours, rate, amount, openShift, run, vendor }) => (
            <div key={c.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: "10px 16px", borderTop: `1px solid ${T.border}22`, fontSize: 12.5, opacity: run ? 0.7 : 1 }}>
              <span style={{ flex: 1, fontWeight: 600, color: T.text }}>{c.name}{vendor && <span style={{ fontWeight: 400, color: T.faint, fontSize: 11 }}> · {vendor}</span>}</span>
              <span style={{ width: 90, textAlign: "right", fontFamily: mono, color: hours > 0 ? T.text : T.faint }}>{run ? run.hours : hours}{openShift && !run ? " ⏱" : ""}</span>
                ...
                {run ? <span style={{ fontFamily: mono, color: T.muted }}>—</span> : (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
                    ...
                    <input value={rateEdits[c.id] !== undefined ? rateEdits[c.id] : (pay[c.id]?.hourly_rate ?? "")} placeholder="0.00" inputMode="decimal"
                      ...
                      style={{ width: 64, padding: "4px 6px", border: `1px solid ${T.border}`, borderRadius: 5, background: T.card, color: T.text, fontSize: 12.5, fontFamily: mono, textAlign: "right", outline: "none" }} />
                  </span>
                )}
              </span>
              <span style={{ width: 110, textAlign: "right", fontFamily: mono, fontWeight: 700, color: T.text }}>{money(run ? run.amount : amount)}</span>
              <span style={{ width: 200, textAlign: "right" }}>
                {run ? <span style={{ color: T.green, fontWeight: 700, fontSize: 12 }}>✓ Pushed{run.qb_doc_number ? ` · #${run.qb_doc_number}` : ""}</span>
                  : <button onClick={() => push(c.id, rate)} disabled={pushing === c.id || hours <= 0 || rate <= 0}
                      style={{ background: hours > 0 && rate > 0 ? T.accent : T.surface, color: hours > 0 && rate > 0 ? "#fff" : T.faint, border: "none", borderRadius: 6, padding: "7px 16px", fontSize: 12, fontWeight: 700, cursor: hours > 0 && rate > 0 ? "pointer" : "default", fontFamily: font }}>
