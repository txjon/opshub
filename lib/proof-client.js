...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes, summaryText, tenantSlug, tenantName }) {
  ...
  // Pre-compute wrapped lines per col so the whole strip can size to
  // the tallest cell — that way a long blank string (e.g. "Comfort
  // Colors 1717 — True Navy") wraps to a 2nd line and the rest of
  // the proof content pushes down instead of getting clipped. Cap at
  // 2 lines to keep the strip from blowing up on pathological input.
  const valueLineHeight = 9;
  const baseStripH = 28;
  const wrapped = metaCols.map((col, i) => {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    const maxValW = colW - (i === 0 ? 10 : 20);
    const lines = doc.splitTextToSize(col.value, maxValW).slice(0, 2);
    return { col, lines, x: marginL + i * colW, maxValW };
  });
  const maxLines = Math.max(1, ...wrapped.map(w => w.lines.length));
  const stripH = baseStripH + (maxLines - 1) * valueLineHeight;

  wrapped.forEach(({ col, lines, x }, i) => {
    ...
    lines.forEach((line, li) => {
      doc.text(line, x + (i === 0 ? 0 : 10), y + 21 + li * valueLineHeight);
    });
  });
  ...
  // ── Mockup image ── (slightly bigger — main visual of the proof)
  if (mockupDataUrl) {
    let imgW = contentW * 0.75;
    let imgH = imgW / 2;
    try {
  ...
  if ((printInfo || []).length > 0) {
    ...
    for (const p of (printInfo || [])) {
      ...
      // Calculate row height \u2014 color swatches render as larger
      // rounded-rectangle pills (label inside) per Jon's direction.
      let rowH = 20;
      if (sizeStr !== '\u2014') rowH += 10;
      ...
      // Color swatches — rounded-rectangle pills (label inside).
      // Bigger than before, but radius small enough to read as a
      // chip not a capsule. Wraps before crossing colEndX so a row
      // of many colors stays inside the column.
      if (colorCount > 0) {
        const pillH = 16;
        const pillRadius = 3;
        const pillHGap = 4;
        ...
        for (const c of sortedColors) {
          ...
          // Wrap to next row if this pill would overflow the column.
          if (pillX + pillW > colEndX && pillX > col2X) {
            pillX = col2X;
            pillY += pillH + pillVGap;
            extraRows++;
          }
          ...
          doc.roundedRect(pillX, pillY, pillW, pillH, pillRadius, pillRadius, 'F');

          if (light) {
            doc.setDrawColor(195, 195, 205);
            doc.setLineWidth(0.5);
            doc.roundedRect(pillX, pillY, pillW, pillH, pillRadius, pillRadius, 'S');
          }

          doc.setTextColor(light ? 36 : 255, light ? 36 : 255, light ? 36 : 255);
          doc.text(c.name, pillX + pillW / 2, pillY + pillH / 2 + pillFontSize * 0.35, { align: 'center' });

          pillX += pillW + pillHGap;
        }

        if (extraRows > 0) rowH += extraRows * (pillH + pillVGap);
      } else {
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.setTextColor(102, 102, 102);
    ...
    // Summary line below table \u2014 custom override wins; empty string
    // omits the bar entirely; null/undefined derives the default.
    y += 4;
    const summaryStr = (summaryText === undefined || summaryText === null)
      ? deriveProofSummary(printInfo, instructions)
      : summaryText;
    ...
    if (summaryStr) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(26, 26, 26);
      doc.text(summaryStr, pageW / 2, y + 12, { align: 'center' });
      y += 20;
    } else {
      y += 8;
    }
