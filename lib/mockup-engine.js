...
// Map print PSD group names to zones
const PLACEMENT_MAP = {
  'Front':      'Full Front',
...
function getTemplate(templatePath) {
  if (cachedTemplatePath === templatePath && cachedTemplate) return cachedTemplate;
  cachedTemplate = readPsd(fs.readFileSync(templatePath));
  cachedTemplatePath = templatePath;
  return cachedTemplate;
}

function getDefaultTemplatePath() {
  return path.join(process.cwd(), 'public', 'templates', 'Tee Mockup Template.psd');
}

function buildMockup(printBuffer, templatePath) {
  if (!templatePath) templatePath = getDefaultTemplatePath();

  const printPsd = readPsd(printBuffer);
  const templatePsd = getTemplate(templatePath);

  const canvas = createCanvas(templatePsd.width, templatePsd.height);
  ...
  for (const group of printPsd.children) {
    ...
    for (const layer of group.children) {
      ...
      // Sample dominant color from the layer
      let hex = '#888888';
      if (layer.canvas) {
        const lCtx = layer.canvas.getContext('2d');
        const lData = lCtx.getImageData(0, 0, layer.canvas.width, layer.canvas.height).data;
        let rSum = 0, gSum = 0, bSum = 0, count = 0;
        for (let i = 0; i < lData.length; i += 40) {
          if (lData[i + 3] > 128) {
            rSum += lData[i];
            gSum += lData[i + 1];
            bSum += lData[i + 2];
    ...
    const artCanvas = createCanvas(artW, artH);
...
module.exports = { buildMockup, getDefaultTemplatePath };
