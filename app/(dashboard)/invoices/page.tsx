  ...
  async function load(depth = 0) {
    try {
      const [jobsRes, itemsRes, decoratorsRes, apRes, entriesRes, marksRes, paysRes, clientsRes, ssRes] = await Promise.all([
        supabase.from("jobs").select("id, job_number, title, phase, client_id, clients(name), payment_terms, target_ship_date, costing_summary, costing_data, type_meta, qb_invoice_number, qb_invoice_id, created_at, shipping_route, fulfillment_status, is_inventory, is_test, is_internal, financial_closed_at"),
