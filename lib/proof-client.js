...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes }) {
  ...
  if (mockupDataUrl) {
    ...
    let imgW = contentW * 0.7;
    let imgH = imgW / 2; // fallback
    try {
  ...
  if ((printInfo || []).length > 0) {
    ...
    for (const p of (printInfo || [])) {
      ...
      let cardH = 20;
      if (p.callout) cardH += 12;
      if ((p.colors || []).length > 0) cardH += 18;
      cardH += 4;

      if (needsNewPage(cardH + 8)) { drawFooter(); doc.addPage(); y = margin; }
      ...
      doc.setFontSize(11);
      doc.setTextColor(30, 34, 50);
      doc.text(p.placement, margin + 14, y + 13);
      ...
      if (sizeStr) {
        ...
        doc.setFontSize(11);
        doc.setTextColor(30, 34, 50);
        doc.text(sizeStr, pageW - margin - 12, y + 13, { align: 'right' });
      }
      ...
      if (p.callout) {
        ...
        doc.text(p.callout, margin + 14, contentY + 2, { maxWidth: contentW - 28 });
        contentY += 12;
      }
