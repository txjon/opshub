...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes }) {
  ...
  // ── Header (matches quote PDF) ──
  let y = 32;
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', marginL, y, 150, 19);
  } else {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.setTextColor(26, 26, 26);
    doc.text('house party distro', marginL, y + 14);
  }
  ...
  doc.text('4670 W Silverado Ranch Blvd, STE 120', marginL, y + 32);
  doc.text('Las Vegas, NV 89139', marginL, y + 43);
  doc.text('hello@housepartydistro.com', marginL, y + 54);
  ...
  metaCols.forEach((col, i) => {
    ...
    doc.text(col.label, x + (i === 0 ? 0 : 10), y + 12);
    ...
    doc.text(valLines[0] || '', x + (i === 0 ? 0 : 10), y + 24);
  });
  ...
  const drawFooter = () => {
    ...
    doc.setFontSize(6);
    doc.setTextColor(170, 170, 170);
    doc.text('HOUSE PARTY DISTRO', marginL, fy);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(102, 102, 102);
    doc.text('hello@housepartydistro.com', marginL, fy + 12);
    doc.text('4670 W Silverado Ranch Blvd, STE 120, Las Vegas, NV 89139', marginL, fy + 22);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(170, 170, 170);
    doc.text('Colors shown are approximate \u2014 refer to Pantone references for exact ink matching.', pageW - marginR, fy + 12, { align: 'right' });
  };
  ...
  if ((printInfo || []).length > 0) {
    ...
    doc.text('PLACEMENT', col1X, y + 4);
    doc.text('COLORS', col2X, y + 4);
    doc.text('SIZE', col3X, y + 4);
    ...
    for (const p of (printInfo || [])) {
      ...
      let rowH = 24;
      if (p.callout) rowH += 12;
      if (colorCount > 0) rowH += 18;
      ...
      if (p.callout) {
        ...
        doc.text(p.callout, col1X + 1, detailY, { maxWidth: (col2X - col1X) - 8 });
      }
      ...
      if (colorCount > 0) {
        ...
        for (const c of (p.colors || [])) {
          ...
          doc.roundedRect(swatchX, swatchY, pillW, pillH, 6, 6, 'F');

          if (light) {
            doc.setDrawColor(190, 190, 200);
            doc.setLineWidth(0.5);
            doc.roundedRect(swatchX, swatchY, pillW, pillH, 6, 6, 'S');
          }
    ...
    doc.setFontSize(7);
    doc.setTextColor(170, 170, 170);
    doc.text('TOTAL', pageW - marginR, y + 8, { align: 'right' });
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(26, 26, 26);
    doc.text(`${totalLocations} location${totalLocations !== 1 ? 's' : ''}  \u00B7  ${totalColors} color${totalColors !== 1 ? 's' : ''}`, pageW - marginR, y + 22, { align: 'right' });

    y += 32;
  }

  // ── Notes section (matches quote notes box style) ──
  if (instructions || notes) {
    if (needsNewPage(60)) { drawFooter(); doc.addPage(); y = 36; }

    const noteText = [instructions ? 'Special Instructions: ' + instructions : '', notes ? 'Notes: ' + notes : ''].filter(Boolean).join('\n');

    // Notes box background
    doc.setFillColor(249, 249, 249);
    const noteLines = doc.splitTextToSize(noteText, contentW - 24);
    const boxH = Math.max(40, noteLines.length * 12 + 28);
    ...
    doc.setFontSize(6);
    doc.setTextColor(170, 170, 170);
    doc.text('NOTES', marginL + 12, y + 14);

    // Notes text
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(85, 85, 85);
    doc.text(noteLines, marginL + 12, y + 26);
