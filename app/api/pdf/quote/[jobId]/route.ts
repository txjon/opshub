...
// Add TEELAND same as STOKED
PRINTERS["TEELAND"] = PRINTERS["STOKED"];
...
function calcCostProduct(p: any, margin: string, inclShip: boolean, inclCC: boolean, allProds: any[]) {
  ...
  const blankCost = (() => {
    if (p.blankCosts && Object.keys(p.blankCosts).length > 0) {
      let total = 0;
      Object.entries(p.blankCosts).forEach(([sz, cost]: any) => { total += (cost || 0) * (p.qtys?.[sz] || 0) * 1.035; });
      return total;
    }
