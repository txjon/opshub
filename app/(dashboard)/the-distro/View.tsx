...
// board's exact data: chain-derived ETAs, stall watch, dock aging).
...
export type ArrivalRow = {
  ...
  eta: string | null;
  etaSource: "carrier" | "human" | "derived" | null;
  deliveredAt?: string | null;
  stall?: { text: string; severe: boolean } | null;
  shipBy?: string | null;
  shippedAt?: string;
...
const PURPLE = "#fd3aa3";
const RADAR_BUCKETS: { key: string; label: string; match: (d: number | null) => boolean }[] = [
...
// delivered-not-received aging: calm <24h · amber 24–48h · red 48h+
function deliveredMeta(iso: string): { text: string; color: string } {
  const ageH = (Date.now() - new Date(iso).getTime()) / 36e5;
  const color = ageH >= 48 ? H.red : ageH >= 24 ? H.amber : H.green;
  const ago = ageH < 1 ? "just now" : ageH < 24 ? `${Math.floor(ageH)}h ago` : `${Math.floor(ageH / 24)}d ago`;
  return { text: `✓ ${ago}`, color };
}
// urgency-coded ETA: red late · amber ≤3d · dim calm · faint TBD; ~ = estimate
function etaMeta(row: ArrivalRow): { text: string; color: string } {
  if (row.deliveredAt) return deliveredMeta(row.deliveredAt);
  if (!row.eta) return { text: row.shipBy === "ASAP" ? "ASAP" : "TBD", color: H.faint };
  const d = daysUntilDay(row.eta);
  const pre = row.etaSource !== "carrier" ? "~" : "";
  if (d != null && d < 0) return { text: `${pre}${fmtDay(row.eta)} · late`, color: H.red };
  if (d === 0) return { text: "today", color: H.amber };
  return { text: `${pre}${fmtDay(row.eta)}`, color: d != null && d <= 3 ? H.amber : "rgba(255,255,255,0.6)" };
}
...
export default function TheDistroView({ rows, drops }: { rows: ArrivalRow[]; drops: DropRow[] }) {
  ...
  const landingsToday = useMemo(() => rows.filter(r => !r.deliveredAt && r.eta && (daysUntilDay(r.eta) ?? 1) <= 0).length, [rows]);
  const onDock = useMemo(() => rows.filter(r => r.deliveredAt).length, [rows]);
  const buckets = useMemo(() => {
    // carrier-delivered boxes leave the date buckets — they are SITTING on the
    // dock unreceived; pinned first, newest-delivered first (receiving2's rule)
    const dock = rows.filter(r => r.deliveredAt).sort((a, b) => (b.deliveredAt || "").localeCompare(a.deliveredAt || ""));
    const rest = [...rows.filter(r => !r.deliveredAt)].sort((a, b) => (a.eta || "9999").localeCompare(b.eta || "9999"));
    const dated = RADAR_BUCKETS.map(bk => ({ ...bk, rows: rest.filter(r => bk.match(r.eta ? daysUntilDay(r.eta) : null)) }));
    return [{ key: "dock", label: "on the dock — not received", rows: dock }, ...dated].filter(bk => bk.rows.length > 0);
  }, [rows]);
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "26px 0 80px" }}>
        ...
        {!loaded ? (
          ...
          <>
            ...
            {/* ── LANDING RADAR — the /distro arrival board, hub skin ── */}
            <section style={{ marginTop: 34 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 12 }}>
                <h2 style={{ margin: 0, fontSize: 19, fontWeight: 900, textTransform: "uppercase", letterSpacing: "-0.01em" }}>Landing.</h2>
                <span style={{ fontSize: 10.5, color: H.faint }}>every box and every strip still at a vendor — plain date = carrier data, ~ = estimate</span>
              </div>
              {buckets.length === 0 && <div style={{ color: H.dim, fontSize: 13 }}>Nothing inbound — no open POs or boxes in transit.</div>}
              {buckets.map(bk => (
                <div key={bk.key} style={{ marginBottom: 18 }}>
                  <div style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: bk.key === "dock" ? H.red : H.faint, margin: "0 0 4px" }}>{bk.label}</div>
                  <div style={{ borderTop: `1px solid ${H.line}` }}>
                    {bk.rows.map(r => {
                      const eta = etaMeta(r);
                      return (
                        <button key={r.id} type="button" onClick={() => setSheet({ kind: "radar", row: r })}
                          style={{ display: "flex", alignItems: "baseline", gap: 14, width: "100%", textAlign: "left", padding: "10px 2px", border: 0, borderBottom: `1px solid ${H.line}`, background: "transparent", cursor: "pointer", color: H.text, font: "inherit" }}>
                          <span style={{ fontSize: 12, fontWeight: 900, fontFamily: H.mono, color: eta.color, width: 92, flexShrink: 0, whiteSpace: "nowrap" }}>{eta.text}</span>
                          <span style={{ flex: 1, minWidth: 0 }}>
                            <span style={{ fontSize: 12.5, fontWeight: 800, textTransform: "uppercase" }}>{r.client}</span>
                            <span style={{ fontSize: 10.5, color: H.faint, marginLeft: 10 }}>
                              {r.kind === "box" ? "in transit" : "at vendor"} · from {r.vendor} · {r.itemsLabel}
                              {r.kind === "strip" && r.shipBy ? ` · ships ${r.shipBy === "ASAP" ? "ASAP" : "~" + fmtDay(r.shipBy)}` : ""}
                            </span>
                            {r.stall && <span style={{ display: "block", fontSize: 9.5, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: r.stall.severe ? H.red : H.amber, marginTop: 2 }}>⚠ {r.stall.text}</span>}
                          </span>
                          <span style={{ fontSize: 12, fontWeight: 900, fontFamily: H.mono, color: "rgba(255,255,255,0.85)", flexShrink: 0 }}>{r.units.toLocaleString()}u</span>
                        </button>
                      );
                    })}
                  </div>
            ...
            {drops.length > 0 && (
              <section style={{ marginTop: 30 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 12 }}>
                  <h2 style={{ margin: 0, fontSize: 19, fontWeight: 900, textTransform: "uppercase", letterSpacing: "-0.01em" }}>Drops.</h2>
                  <span style={{ fontSize: 10.5, color: H.faint }}>what the shop has scheduled — plan dock time around the close dates</span>
                </div>
  ...
  return (
    <div className="ds-sheet-wrap" onClick={onClose}>
      <div className="ds-sheet" onClick={e => e.stopPropagation()}>
        ...
        {sheet.kind === "radar" && (() => {
          const r: ArrivalRow = sheet.row;
          const eta = etaMeta(r);
          return (
            <>
              {head(`${r.client} · ${r.kind === "box" ? "in transit" : "at vendor"}`,
                r.kind === "box" ? "Landing" : "In the works",
                `from ${r.vendor} · ${r.units.toLocaleString()} u · ${r.deliveredAt ? "delivered " : "ETA "}${eta.text}`,
                eta.color === H.faint ? "#fff" : eta.color)}
              {r.kind === "box" && (
                <div style={{ fontSize: 10.5, fontFamily: H.mono, color: "rgba(255,255,255,0.65)", marginBottom: 12 }}>
                  {r.pickup ? "pickup" : <>{r.carrier || ""} {r.tracking ? <TrackingLink tracking={r.tracking} shipmentId={r.id} /> : "no tracking on file"}</>}
                </div>
              )}
              {r.stall && <div style={{ fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: r.stall.severe ? H.red : H.amber, marginBottom: 12 }}>⚠ {r.stall.text}</div>}
              {r.note && <div style={{ fontSize: 12, borderLeft: `3px solid ${PURPLE}`, paddingLeft: 10, color: "rgba(255,255,255,0.85)", marginBottom: 12, lineHeight: 1.5 }}>{r.note}</div>}
              {(r.slips || []).length > 0 && (
                <div style={{ marginBottom: 12 }}>{(r.slips || []).map((s, i) => (
                  <a key={i} href={s.url} target="_blank" rel="noreferrer" style={{ fontSize: 11.5, color: H.blue, textDecoration: "none", marginRight: 14 }}>📎 {s.name}</a>
                ))}</div>
              )}
              <div style={{ marginBottom: 16 }}>
                {r.lines.map((l, i) => (
                  <div key={i} style={{ padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "baseline" }}>
                      <span style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.name}</span>
                      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: H.faint, flexShrink: 0 }}>{(l.route || "").replace(/_/g, " ")}</span>
                      <span style={{ fontSize: 11.5, fontWeight: 900, fontFamily: H.mono, flexShrink: 0 }}>{sum(l.qtys)}</span>
                    </div>
                    <div style={{ fontSize: 10, fontFamily: H.mono, color: "rgba(255,255,255,0.55)", marginTop: 3 }}>
                      {Object.entries(l.qtys || {}).map(([k, v]) => `${k} ${v}`).join("  ")}
                    </div>
                    {r.kind === "box" && l.receivedQtys && sum(l.receivedQtys) > 0 && (
                      <div style={{ fontSize: 10, color: H.amber, fontWeight: 700, marginTop: 3 }}>{sum(l.receivedQtys)} already counted in</div>
                    )}
                    {r.kind === "strip" && l.shippedTotal != null && l.shippedTotal > 0 && (
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", marginTop: 3 }}>{l.shippedTotal} of {l.orderedTotal} already shipped — these are the owed units</div>
                    )}
                  </div>
                ))}
              </div>
              <a href={r.kind === "box" ? "/receiving2" : "/production2"} style={{ ...goBtn(true), display: "inline-block", textDecoration: "none" }}>
                {r.kind === "box" ? "Act on receiving →" : "Act on production →"}
              </a>
            </>
          );
        })()}
