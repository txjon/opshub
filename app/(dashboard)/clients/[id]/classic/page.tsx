...
import { STATE_LABELS, jobPhaseToItemState, type ItemState } from "@/lib/item-status";
import { ClientWorkingSheet, flattenClientItems, resolveWsState, ITEM_STATE_COLORS } from "@/components/ClientWorkingSheet";
...
export default function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    const [cRes, ctRes, jRes, rRes] = await Promise.all([
      ...
      supabase.from("jobs").select("*, costing_summary, type_meta, shipping_route, phase_timestamps, items(id, name, blank_vendor, blank_sku, cost_per_unit, sell_per_unit, blank_costs, sort_order, pipeline_stage, working_status, client_retail_per_unit, client_eta, notes, received_at_hpd, blanks_order_cost, archived_at, completed_at, shipping_route, forwarded_at, webstore_entered_at, decorator_assignments(decorators(name, short_code)), buy_sheet_lines(size, qty_ordered)), payment_records(amount, status, due_date)").eq("client_id", params.id).order("created_at", { ascending: false }),
    ...
    setLoading(false);
    loadFiles();
    // Worksheet thumbnails now load inside ClientWorkingSheet.
  }
  ...
  // Flattening lives in ClientWorkingSheet (shared with the client space) —
  // one shape for the worksheet, the Orders hover, and History.
  const allItems = flattenClientItems(jobs);
  ...
  // Per-item canonical status — hoisted so the Orders row hover shows the
  // same state vocabulary the worksheet does. Map keyed by id for O(1).
  const wsItemsAll = allItems.map((it: any) => ({ ...it, _ws: resolveWsState(it) }));
  const itemStateById = new Map<string, ItemState>();
  for (const it of wsItemsAll) itemStateById.set(it.id, it._ws);

  // Worksheet edits land here so every panel on this page (KPI strip,
  // History, Orders hover) recomputes from the same jobs state.
  function onWorksheetItemChange(itemId: string, field: string, value: any) {
  ...
  return (
    <div style={{fontFamily:font,color:T.text,maxWidth:900,margin:"0 auto",paddingBottom:"3rem"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        {/* Working Sheet — extracted to components/ClientWorkingSheet (Jul 28,
            shared with the client space's Pipeline section). Summary card +
            full-screen modal behavior lives in the component (variant="card"). */}
        <ClientWorkingSheet
          variant="card"
          clientId={params.id}
          clientName={client?.name || ""}
          jobs={jobs}
          onItemLocalChange={onWorksheetItemChange}
        />
      </div>
