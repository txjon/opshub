...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes, tenantSlug, tenantName }) {
  ...
  // Logo — try preloaded PNG, fallback to text. Width derived from
  // the cached aspect ratio so per-tenant logos render proportionally
  // (HPD's wide wordmark vs IHM's squarer mark).
  if (cached?.dataUrl) {
    const logoH = 18;
    const logoW = Math.min(180, Math.round(logoH * (cached.ratio || 8)));
