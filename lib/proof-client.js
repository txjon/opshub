...
// Render SVG logo to a data URL for embedding in PDF
...
function getLogoDataUrl() {
  ...
  const scale = 4; // render at 4x for sharpness
  canvas.width = 227 * scale;
  canvas.height = 29 * scale;
  const ctx = canvas.getContext('2d');
  const img = new Image();
  img.src = url;
  // Synchronous draw won't work — need to pre-load
  // Fall back to returning null, will use text
  return null;
}
...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor }) {
  ...
  // ── Header — white background with SVG logo ──
  let y = 30;
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', margin, y, 170, 21);
  } else {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.setTextColor(26, 26, 26);
    doc.text('house party distro', margin, y + 16);
  }

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(18);
  doc.setTextColor(26, 26, 26);
  doc.text('Print Proof', pageW - margin, y + 8, { align: 'right' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(138, 146, 176);
  doc.text(today, pageW - margin, y + 22, { align: 'right' });
  ...
  // ── Info row — 3 columns (no decorator) with wider client ──
  const infoCols = [
    { label: 'CLIENT', value: clientName || '—', flex: 1.5 },
    { label: 'ITEM', value: itemName || '—', flex: 1 },
    { label: 'BLANK', value: blankStr, flex: 1.5 },
  ];
  ...
  infoCols.forEach((col, i) => {
    const w = (col.flex / totalFlex) * contentW;
    const x = colX + 8;
    if (i > 0) {
      doc.line(colX, y, colX, y + 36);
    }
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6.5);
    ...
    doc.setFontSize(9);
    doc.setTextColor(26, 26, 26);
    doc.text(col.value, x, y + 26, { maxWidth: w - 16 });
  ...
  // ── Mockup image — white background ──
  if (mockupDataUrl) {
    const imgW = contentW;
    const imgH = contentW / 2;
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(margin, y, imgW, imgH + 24, 3, 3, 'F');
    doc.addImage(mockupDataUrl, 'JPEG', margin + 12, y + 12, imgW - 24, imgH);
    y += imgH + 36;
  }
  ...
  // ── Print placements (in PSD source order) ──
  for (const p of (printInfo || [])) {
    ...
    // Color swatches — centered text in pills
    ...
    for (const c of (p.colors || [])) {
      ...
      doc.roundedRect(swatchX, swatchY, pillW, pillH, 8, 8, 'F');

      if (light) {
        doc.setDrawColor(200, 200, 200);
        doc.setLineWidth(0.5);
        doc.roundedRect(swatchX, swatchY, pillW, pillH, 8, 8, 'S');
      }

      // Circle swatch
      doc.circle(swatchX + 10, swatchY + pillH / 2, 4, 'F');

      // Color name — centered in pill
      doc.setTextColor(light ? 34 : 255, light ? 34 : 255, light ? 34 : 255);
      const textX = swatchX + 20 + (pillW - 28) / 2;
      doc.text(c.name, textX, swatchY + pillH / 2 + 2.8, { align: 'center' });
