...
export async function buildMockupClient(psdArrayBuffer) {
  ...
  for (const group of groups) {
    ...
    for (const layer of group.children) {
      ...
      const excludeFromBounds = isHidden || isBlank || isNonInk;
      ...
      // Only list real ink layers as color separations — hidden/blank/non-ink are not pulls
      if (!isHidden && !isBlank && !isNonInk) {
