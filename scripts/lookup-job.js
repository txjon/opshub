...
(async () => {
  const { data, error } = await supabase.from("jobs").select("id, title, job_number, type_meta, qb_invoice_number, qb_invoice_id").eq("job_number", jobNumber).single();
  if (error || !data) { console.error("Not found:", error?.message); process.exit(1); }
  console.log(JSON.stringify({
    id: data.id,
    title: data.title,
    job_number: data.job_number,
    qb_invoice_id: data.qb_invoice_id || null,
    qb_invoice_number: data.qb_invoice_number || null,
