...
export async function packetHtml(wo: DesignWorkOrder, t: ResolvedTarget, messages: any[]): Promise<string> {
  ...
  const header = `<div class="head"><div><div class="eyebrow">Work order · ${esc(t.companyName)}</div><h1>${esc(what)}</h1><div class="meta">${esc(woTypeLabel(wo.type))}${due ? ` · due ${esc(due)}` : ""}${wo.designer_name ? ` · for ${esc(wo.designer_name)}` : ""} · ${new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" , timeZone: "America/Los_Angeles" })}</div></div>${wo.headline ? `<div class="headline">${esc(wo.headline)}</div>` : ""}</div>`;
