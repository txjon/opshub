...
// the bundle anyway and the strings aren't large enough to warrant a
// separate fetch. Other branding (addresses, emails, name, legal name)
// reads from the active company's row.
//
// Adding a new tenant: drop their SVG into LOGOS_BY_SLUG and seed
// their companies row with addresses + emails. PDF routes pick it
// up automatically via getPdfBranding().

// HPD's wordmark — used on Quote / Invoice / PO / RFQ PDF headers.
// Originally hardcoded inline in each PDF route.
...
export type PdfBranding = {
  /** Display name e.g. "House Party Distro" */
  name: string;
  /** Inline SVG ready to drop into a PDF template */
  logoSvg: string;
  /** Header address pre-formatted with <br/> line breaks. Prefers
   *  warehouse_address (HPD has one); falls back to bill_to_address
   *  (IHM has no warehouse, only bills). */
...
// Newlines win when present (DB-stored multi-line). Fallback heuristic
// splits before the city (last segment that looks like "City, ST ZIP").
function addressToHtml(addr: string | null): string {
  if (!addr) return "";
  if (addr.includes("\n")) {
    return addr.split(/\n+/).map(s => s.trim()).filter(Boolean).join("<br/>");
  }
  // Heuristic: split on the last ", " that comes before a state/zip.
  // Falls back to single-line if we can't find the cut point.
