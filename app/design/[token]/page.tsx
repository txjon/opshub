...
// ?t= carries the designer's own link token so the file routes know who is
// asking (lib/file-access).
const thumbUrl = (id: string | null | undefined, token?: string | null) => id ? `/api/files/thumbnail?id=${id}&thumb=1${token ? `&t=${encodeURIComponent(token)}` : ""}` : null;
    ...
    onMouseLeave={(e: any) => { e.currentTarget.style.boxShadow = "none"; }}>
      ...
      {brief.has_unread_external && !isClientAborted && highlight && (
        <div style={{
          ...
          <span style={{ flexShrink: 0, marginTop: 1, fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 5px", borderRadius: 2, background: event ? "rgba(0,0,0,0.18)" : C.purple, color: "#fff" }}>{ribbonLabel}</span>
