...
async function verifyAccess(token: string, briefId: string) {
  ...
  const { data: brief } = await db.from("art_briefs").select("id, title, state, assigned_designer_id, job_id, client_aborted_at").eq("id", briefId).single();
  // Aborted (client) or archived (HPD) briefs are recalled from the
  // designer side — same as if they were never assigned.
