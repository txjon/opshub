...
function sortSizes(items) {
  return [...items].sort((a, b) => {
    const ai = SIZE_ORDER.indexOf(a.name), bi = SIZE_ORDER.indexOf(b.name);
...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes }) {
  ...
  let y = 28;
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', marginL, y, 140, 18);
  } else {
  ...
  // Address below logo (compact: 2 lines)
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(102, 102, 102);
  doc.text('4670 W Silverado Ranch Blvd, STE 120, Las Vegas, NV 89139', marginL, y + 28);
  doc.text('hello@housepartydistro.com', marginL, y + 38);
  ...
  const drawFooter = () => {
    ...
    doc.setFontSize(7);
    doc.setTextColor(102, 102, 102);
    doc.text('hello@housepartydistro.com  \u00B7  4670 W Silverado Ranch Blvd, STE 120, Las Vegas, NV 89139', marginL, fy + 10);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.5);
    doc.setTextColor(170, 170, 170);
    doc.text('Colors shown are approximate \u2014 refer to Pantone references for exact ink matching.', pageW - marginR, fy + 10, { align: 'right' });
  };
  ...
  if ((printInfo || []).length > 0) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(170, 170, 170);
    doc.text('TOTAL', pageW - marginR, y + 6, { align: 'right' });
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(26, 26, 26);
    doc.text(`${totalLocations} location${totalLocations !== 1 ? 's' : ''}  \u00B7  ${totalColors} color${totalColors !== 1 ? 's' : ''}`, pageW - marginR, y + 18, { align: 'right' });
  ...
  if (instructions || notes) {
    ...
    const noteLines = doc.splitTextToSize(noteText, contentW - 20);
