...
function renderPOHTML(data: any): string {
  ...
  const itemBlocks = data.items.map((item: any, idx: number) => {
    const lines = sortSizes(Object.keys(item.qtys).filter(sz => (item.qtys[sz] || 0) > 0));
    ...
    return `<div style="border-left:3px solid #1a1a1a;padding-left:16px;margin-bottom:24px">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px">
        <div style="font-size:13px;font-weight:700">${String.fromCharCode(65 + idx)} — ${item.name}</div>
      ...
      <div style="display:flex;gap:16px;margin-bottom:8px;font-size:10px;color:#555">
        ...
        ${item.decoration_type ? `<div><span style="font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#bbb;margin-right:4px">Deco</span>${item.decoration_type}</div>` : ""}
      </div>
...
<div style="background:#fff;font-family:${font};color:#111;max-width:780px;margin:0 auto">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:18px;border-bottom:2px solid #1a1a1a;margin-bottom:18px">
    ...
    <div style="text-align:right">
      <div style="font-size:20px;font-weight:800;letter-spacing:-0.5px;color:#1a1a1a;font-family:${font}">PO# ${data.job_number || "—"}</div>
  ...
  <div style="display:flex;gap:0;border:0.5px solid #ccc;margin-bottom:16px">
    ${[
      ["Date", today],
      ["Ship date", shipDate],
      ["Vendor ID", data.vendor_short_code || data.vendor_name],
      ["Ship method", data.ship_method || "—"],
      ["Ship acct.", data.ship_account || "—"],
    ].map(([k, v], i, arr) =>
      `<div style="flex:1;padding:5px 8px;${i < arr.length - 1 ? "border-right:0.5px solid #ccc" : ""}">
        <div style="font-size:7.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#aaa;margin-bottom:2px">${k}</div>
        <div style="font-size:10px;font-weight:600;color:#1a1a1a">${v}</div>
      </div>`
    ).join("")}
  </div>
...
export async function GET(req: NextRequest, { params }: { params: { jobId: string } }) {
  try {
    ...
    // Fetch items with all PO-relevant fields
    ...
    if (itemsError) {
      return NextResponse.json({ error: "Failed to fetch items", detail: itemsError?.message }, { status: 500 });
    }
    ...
    const mappedItems = (items || []).map((it: any) => {
      ...
      const cp = costProds.find(p => p.id === it.id);
      const supplier = cp?.supplier || "";
      const printVendor = cp?.printVendor || "";
    ...
    if (vendorItems.length === 0) {
      return NextResponse.json({ error: "No items found for this vendor" }, { status: 404 });
    }

    // Get decorator info from first item that has it
    ...
    // Get ship method from costing data orderInfo
    const orderInfo = costingData.orderInfo || {};

    // Fetch decorators table for ship method/account if available
    const { data: decoratorRecord } = await supabase
      .from("decorators")
      .select("*")
      .ilike("name", vendorName)
      .single().then(r => r).catch(() => ({ data: null, error: null }));
