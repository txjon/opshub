...
export async function buildMockupClient(psdArrayBuffer) {
  ...
  // Resize to half for smaller file sizes (1400x700 is plenty for display + PDF)
  const outW = Math.round(canvas.width / 2);
  const outH = Math.round(canvas.height / 2);
  ...
  // PNG for display and Drive upload
  const pngDataUrl = outCanvas.toDataURL('image/png');
  const mockupBase64 = pngDataUrl.split(',')[1];
  const pngBlob = await new Promise(resolve => outCanvas.toBlob(resolve, 'image/png'));

  // Smaller JPEG for embedding in proof PDF
  const jpegDataUrl = outCanvas.toDataURL('image/jpeg', 0.8);
  const jpegBase64 = jpegDataUrl.split(',')[1];

  return { mockupBase64, pngBlob, jpegBase64, printInfo };
}
