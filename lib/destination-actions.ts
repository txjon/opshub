...
export async function setDefaultLocation(sb: Sb, clientId: string, id: string): Promise<void> {
  ...
  const { error } = await sb.from("client_locations").update({ is_default: true }).eq("id", id);
  if (error) throw new Error(error.message);
}
...
export async function setJobShipTo(sb: Sb, jobId: string, loc: LocationRow): Promise<void> {
