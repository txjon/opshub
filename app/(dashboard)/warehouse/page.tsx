          ...
          {shipThrough.map(job => (
            <div key={job.id} style={{ ...card, marginBottom: 8 }}>
              <div style={{ padding: "10px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 8 }}>
                <Link href={`/jobs/${job.id}`} style={{ fontSize: 13, fontWeight: 600, color: T.text, textDecoration: "none" }}>{job.client_name}</Link>
                <span style={{ fontSize: 11, color: T.muted }}>— {job.title}</span>
                <span style={{ marginLeft: "auto", fontSize: 11, color: T.green, fontWeight: 600 }}>All {job.items.length} items received — ready to ship</span>
              </div>
              <div style={{ padding: "10px 14px", fontSize: 12, color: T.muted }}>
                Forward to client. Mark complete on the project page when shipped.
              </div>
            </div>
          ))}
        </div>
