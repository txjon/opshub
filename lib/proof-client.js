...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes }) {
  ...
  let y = 30;
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', margin, y, 150, 19);
  } else {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.setTextColor(26, 26, 26);
    doc.text('house party distro', margin, y + 14);
  }

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(26, 26, 26);
  doc.text('PRINT PROOF', pageW - margin, y + 6, { align: 'right' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(138, 146, 176);
  doc.text(today, pageW - margin, y + 20, { align: 'right' });

  y = 92;

  // ── Info cards ──
  const infoCols = [
    { label: 'CLIENT', value: clientName || '\u2014', flex: 1.2 },
    { label: 'ITEM', value: itemName || '\u2014', flex: 1 },
    { label: 'BLANK', value: blankStr, flex: 1.4 },
    ...(method ? [{ label: 'METHOD', value: method, flex: 0.8 }] : []),
  ];
  const totalFlex = infoCols.reduce((a, c) => a + c.flex, 0);
  const cardH = 42;
  const gap = 8;

  let colX = margin;
  infoCols.forEach((col, i) => {
    const w = (col.flex / totalFlex) * (contentW - gap * (infoCols.length - 1));

    // Card background
    doc.setFillColor(245, 246, 250);
    doc.roundedRect(colX, y, w, cardH, 4, 4, 'F');
    ...
    doc.text(col.value, colX + 12, y + 30, { maxWidth: w - 24 });
  ...
  const drawFooter = () => {
    ...
    doc.setTextColor(160, 165, 185);
    doc.text(
      'This proof represents print placement, sizing, and ink colors for the specified item. Colors shown are approximate \u2014 refer to Pantone references for exact ink matching.',
      margin, fy, { maxWidth: contentW * 0.7 }
    );
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(160, 165, 185);
    doc.text('House Party Distro', pageW - margin, fy, { align: 'right' });
  };

  if ((printInfo || []).length > 0) {
    // Check if print details header + at least one card fits
    if (needsNewPage(80)) { drawFooter(); doc.addPage(); y = margin; }
    ...
    doc.text('PRINT DETAILS', margin + 12, y + 14);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(138, 146, 176);
    doc.text(`${totalLocations} location${totalLocations !== 1 ? 's' : ''}  \u00B7  ${totalColors} color${totalColors !== 1 ? 's' : ''}`, pageW - margin - 12, y + 14, { align: 'right' });
    ...
    for (const p of (printInfo || [])) {
      ...
      const sizeStr = (p.widthInches && p.heightInches) ? `${p.widthInches}" \u00D7 ${p.heightInches}"` : '';
      let cardH = 26;
      if (p.callout) cardH += 14;
      if ((p.colors || []).length > 0) cardH += 22;
      cardH += 8;

      if (needsNewPage(cardH + 14)) { drawFooter(); doc.addPage(); y = margin; }

      // Card background
      doc.setFillColor(248, 249, 252);
      doc.roundedRect(margin, y, contentW, cardH, 5, 5, 'F');
      doc.setFillColor(30, 34, 50);
      ...
      doc.setFontSize(11);
      doc.setTextColor(30, 34, 50);
      doc.text(p.placement, margin + 14, y + 16);
      ...
      if (sizeStr) {
        ...
        doc.setFontSize(11);
        doc.setTextColor(30, 34, 50);
        doc.text(sizeStr, pageW - margin - 12, y + 16, { align: 'right' });
      }
      ...
      if (p.callout) {
        ...
        doc.text(p.callout, margin + 14, contentY + 4, { maxWidth: contentW - 28 });
        contentY += 14;
      }

      // Color swatches
      if ((p.colors || []).length > 0) {
        let swatchX = margin + 14;
        for (const c of (p.colors || [])) {
          ...
          doc.roundedRect(swatchX, contentY, pillW, pillH, 7, 7, 'F');

          if (light) {
            doc.setDrawColor(190, 190, 200);
            doc.setLineWidth(0.5);
            doc.roundedRect(swatchX, contentY, pillW, pillH, 7, 7, 'S');
          }

          doc.setTextColor(light ? 40 : 255, light ? 40 : 255, light ? 40 : 255);
          doc.text(c.name, swatchX + pillW / 2, contentY + pillH / 2 + 2.5, { align: 'center' });
  ...
  if (instructions || notes) {
    if (needsNewPage(60)) { drawFooter(); doc.addPage(); y = margin; }
    ...
    if (instructions) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(50, 54, 70);
      const instrLines = doc.splitTextToSize(instructions, contentW);
      doc.text(instrLines, margin, y + 22);
      y += 24 + instrLines.length * 12;
    }
    ...
    if (notes) {
      ...
      doc.text(noteLines, margin, y + 22);
      y += 24 + noteLines.length * 12;
    }
  }
