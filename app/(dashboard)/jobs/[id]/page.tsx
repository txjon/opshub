        ...
        <div style={{position:"relative"}}>
          ...
          {ovMenu && (
            <div style={{position:"absolute",top:"100%",right:0,marginTop:6,background:T.card,border:`1px solid ${T.border}`,borderRadius:10,boxShadow:"0 10px 30px rgba(0,0,0,.16)",zIndex:40,minWidth:196,overflow:"hidden"}}>
              ...
              {/* Un-cancel: cancelled was a dead-end — no action moved a job out of it (Jon's audit, Jul 22). Reactivate drops it back to intake and recalcs; a voided QB invoice stays voided (re-invoice if needed). */}
              {job.phase==="cancelled"&&<button onClick={async()=>{setOvMenu(false);await supabase.from("jobs").update({phase:"intake"} as never).eq("id",job.id);setJob(j=>j?{...j,phase:"intake"} as any:j);setTimeout(recalcPhase,300);}} style={{display:"block",width:"100%",textAlign:"left",padding:"9px 13px",background:"none",border:"none",cursor:"pointer",fontFamily:font,fontSize:12.5,fontWeight:600,color:T.green}}>Reactivate project</button>}
