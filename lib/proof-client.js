...
export function generateProofPdfClient({ mockupDataUrl, printInfo, clientName, itemName, blankVendor, blankStyle, blankColor, method, instructions, notes, summaryText, finishing, addOns, fleece, colorCount, locationCount, tenantSlug, tenantName }) {
  ...
  // \u2500\u2500 Meta cards: Item \u00b7 Blank \u2500\u2500
  boxRow([['ITEM', itemName || '\u2014'], ['BLANK', blankStr]], 11);
  // ── Mockup image ── (mirrors the smaller web mockup) ──
  if (mockupDataUrl) {
    let imgW = contentW * 0.58;
    let imgH = imgW / 2;
    try {
    ...
    if (needsNewPage(imgH + 14)) { drawFooter(); doc.addPage(); y = 36; }
  ...
  // -- Product spec (KPI tiles, mirrors web) --
  {
    const tiles = [
      ['Method', method || '—'],
      ['Colors', colorCount ? String(colorCount) : '—'],
