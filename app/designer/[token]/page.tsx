...
import { woTypeLabel, isProductionType, type BriefSpec } from "@/lib/design-work-orders";
...
export default function DesignerPage({ params }: { params: { token: string } }) {
  ...
  // Files with no preview (layered PSD): the tile downloads directly instead
  // of opening an empty lightbox.
  ...
  // Production orders (seps / vector): files only — no PDF brief, no packet.
  const production = isProductionType(wo.type);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "28px 14px 90px", overflowX: "hidden", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginTop: 16 }}>
        <a href={`/api/designer/${token}/package`} style={{ ...primaryBtn, textDecoration: "none", display: "inline-block", padding: "11px 18px" }}>{production ? "↓ Download all files (.zip)" : "↓ Download package (.zip)"}</a>
        {!production && <a href={`/api/designer/${token}/packet`} style={{ ...ghostBtn, textDecoration: "none", display: "inline-block", padding: "11px 16px" }}>PDF brief</a>}
      </div>
      <div style={{ textAlign: "center", fontSize: 10.5, color: H.faint, marginTop: 6 }}>{production ? "Every file at full resolution. Upload your work below." : "Every file at full resolution + the brief as a PDF, for working offline. Upload your work below."}</div>
      ...
      {spec.extras.length > 0 && (
        <section style={{ marginTop: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: spec.canvases.length ? "repeat(auto-fill, minmax(92px, 1fr))" : "repeat(auto-fill, minmax(140px, 1fr))", gap: 8 }}>
            {spec.extras.map(e => (
              <button key={e.driveId} type="button" onClick={() => { if (noPreview.has(e.driveId)) { window.location.href = dl(e.driveId); return; } setLit({ src: img(e.previewId || e.driveId, 1600), downloadHref: dl(e.driveId), name: e.name, caption: e.label || "Reference" }); }} style={{ display: "block", aspectRatio: "1", borderRadius: 10, overflow: "hidden", background: "#fff", position: "relative", border: "none", padding: 0, cursor: "zoom-in", width: "100%" }}>
                ...
                <img src={img(e.previewId || e.driveId, 400)} alt="" loading="lazy" referrerPolicy="no-referrer" style={{ position: "relative", width: "100%", height: "100%", objectFit: "cover" }} onError={(ev: any) => { ev.target.style.display = "none"; setNoPreview(prev => new Set(prev).add(e.driveId)); }} />
