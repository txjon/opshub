      ...
      {tab==="overview"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16,alignItems:"start"}}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={card}>
              <div style={{fontSize:11,fontWeight:500,color:"var(--color-text-secondary)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>Job info</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div><label style={lc}>Job title</label><input style={ic} value={job.title} onChange={e=>upd("title",e.target.value)}/></div>
                <div><label style={lc}>Job type</label>
              ...
              <div style={{marginTop:12}}><label style={lc}>Notes</label><textarea style={{...ic,minHeight:70,resize:"vertical",lineHeight:1.5}} value={job.notes||""} onChange={e=>upd("notes",e.target.value)}/></div>
            </div>
            {job.job_type==="tour"&&(
              <div style={card}>
                <div style={{fontSize:11,fontWeight:500,color:"var(--color-text-secondary)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>Tour details</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  <div><label style={lc}>Venue</label><input style={ic} value={job.type_meta?.venue_name||""} onChange={e=>upd("type_meta",{...job.type_meta,venue_name:e.target.value})}/></div>
                  <div><label style={lc}>Show date</label><input style={ic} type="date" value={job.type_meta?.show_date||""} onChange={e=>upd("type_meta",{...job.type_meta,show_date:e.target.value})}/></div>
                  <div style={{gridColumn:"1/-1"}}><label style={lc}>Venue address</label><input style={ic} value={job.type_meta?.venue_address||""} onChange={e=>upd("type_meta",{...job.type_meta,venue_address:e.target.value})}/></div>
                </div>
              </div>
            )}
            {payments.length>0&&(
          ...
          <div style={card}>
            ...
            {contacts.map(c=>(
              <div key={c.id} style={{display:"flex",alignItems:"flex-start",gap:10,paddingBottom:12,marginBottom:12,borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
                <div style={{width:36,height:36,borderRadius:"50%",background:"var(--color-background-info)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:500,color:"var(--color-text-info)",flexShrink:0}}>
                  {c.name.split(" ").map((n:string)=>n[0]).join("").slice(0,2)}
                </div>
                <div>
                  <div style={{fontSize:13,fontWeight:500}}>{c.name}</div>
                  <div style={{fontSize:11,color:"var(--color-text-secondary)"}}>{c.role_label} · {c.role_on_job}</div>
                  {c.email&&<div style={{fontSize:11,color:"var(--color-text-info)",marginTop:2}}>{c.email}</div>}
                </div>
              </div>
            ))}
            <a href={`/jobs/${params.id}/items/new`} style={{display:"block",marginTop:8,fontSize:12,color:"var(--color-text-info)",textDecoration:"none"}}>+ Add item</a>
          </div>
        </div>
      )}
