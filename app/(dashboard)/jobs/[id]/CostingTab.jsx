...
// Active pricing map — populated from DB decorators, with fallback
export let PRINTERS = {...FALLBACK_PRINTERS};
...
export function loadPricingFromDecorators(decorators) {
  ...
  for (const d of decorators) {
    ...
    if (d.pricing_data) {
      map[key] = d.pricing_data;
    } else if (FALLBACK_PRINTERS[key]) {
      map[key] = FALLBACK_PRINTERS[key];
    } else {
      // Decorator exists but no pricing — add with empty structure so it shows in dropdown
      map[key] = { qtys:[], prices:{}, tagPrices:[], finishing:{}, setup:{Screens:0,TagScreens:0,Seps:0,InkChange:0}, specialty:{} };
    }
  }
  PRINTERS = map;
  return map;
