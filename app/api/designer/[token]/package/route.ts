...
import { packageZip } from "@/lib/design-work-orders-packet";
...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  ...
  try {
    const { buf, name } = await packageZip(r.wo, t, r.messages);
    return new NextResponse(buf as any, { headers: { "Content-Type": "application/zip", "Content-Disposition": `attachment; filename="${name.replace(/[^\x20-\x7E]/g, "_")}"; filename*=UTF-8''${encodeURIComponent(name)}`, "Cache-Control": "no-store" } });
  } catch (e: any) {
