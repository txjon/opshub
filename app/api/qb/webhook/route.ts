...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[QB Webhook1] POST received, body length:", rawBody.length);

    if (!verifySignature(rawBody, signature)) {
      console.error("[QB Webhook1] HMAC signature mismatch");
    ...
    console.log("[QB Webhook1] Notifications:", notifications.length);

    for (const notification of notifications) {
      const realmId = notification.realmId;
      if (realmId !== process.env.QB_REALM_ID) {
        console.error("[QB Webhook1] Realm mismatch:", realmId, "!==", process.env.QB_REALM_ID);
        continue;
      }

      const events = notification.dataChangeEvent?.entities || [];
      console.log("[QB Webhook1] Entities:", events.map((e: any) => `${e.name}:${e.id}`).join(", "));

      for (const entity of events) {
        if (entity.name !== "Payment") continue;
        const paymentId = entity.id;
        console.log("[QB Webhook1] Processing payment:", paymentId);
        ...
        try {
          ...
          console.error("[QB Webhook1] Token refresh FAILED:", e.message);
          continue;
        }
        ...
        if (!res.ok) {
          ...
          console.error("[QB Webhook1] QB API error fetching payment:", res.status, errText.slice(0, 300));
          ...
          try {
            ...
            if (!retry.ok) {
              console.error("[QB Webhook1] Retry also failed:", retry.status);
              continue;
            }
            const retryData = await retry.json();
            await processPayment(retryData.Payment, supabase, paymentId);
          } catch (retryErr: any) {
            console.error("[QB Webhook1] Retry exception:", retryErr.message);
          }
          continue;
        }
    ...
    console.error("[QB Webhook1] Top-level error:", e.message, e.stack?.slice(0, 300));
    return NextResponse.json({ success: true });
  }
}
...
async function processPayment(payment: any, supabase: any, paymentId: string) {
  if (!payment) {
    console.error("[QB Webhook1] Payment object is null for ID:", paymentId);
    return;
  }

  const amount = payment.TotalAmt || 0;
  console.log("[QB Webhook1] Payment amount:", amount, "CustomerRef:", payment.CustomerRef?.value);
  ...
  console.log("[QB Webhook1] Linked invoice IDs:", invoiceRefs.join(", ") || "NONE");

  if (invoiceRefs.length === 0) {
    // Fallback: try to match by customer name or payment memo
    console.error("[QB Webhook1] No linked invoices found — payment cannot be matched");
    return;
  }
  ...
  for (const qbInvoiceId of invoiceRefs) {
    ...
    if (!jobs?.length) {
      console.error("[QB Webhook1] NO JOB FOUND for QB invoice:", qbInvoiceId);
      continue;
    }
    ...
    if (insertErr) {
      console.error("[QB Webhook1] INSERT FAILED:", insertErr.message, insertErr.details);
      continue;
    }
    ...
    console.log(`[QB Webhook1] SUCCESS — $${amount} recorded for "${job.title}" (${job.id})`);
  }
}
